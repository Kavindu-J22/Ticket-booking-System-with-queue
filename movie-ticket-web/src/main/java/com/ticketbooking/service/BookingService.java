...
public class BookingService {
    ...
    public Booking updateBooking(Booking booking) {
        ...
        if (existingBooking.isPresent()) {
            ...
            // If number of tickets has changed, update available seats
            if (oldBooking.getNumberOfTickets() != booking.getNumberOfTickets() &&
                    oldBooking.getStatus().equals("CONFIRMED") &&
                    booking.getStatus().equals("CONFIRMED")) {

                // Calculate the difference in tickets
                int ticketDifference = booking.getNumberOfTickets() - oldBooking.getNumberOfTickets();

                // Update available seats
                if (ticketDifference != 0) {
                    try {
                        movieService.updateAvailableSeats(booking.getMovieId(), ticketDifference);
                    } catch (Exception e) {
                        throw new IllegalStateException("Failed to update available seats: " + e.getMessage());
                    }
                }
            }

            // If show time has changed, recalculate total price
            if (!oldBooking.getShowTime().equals(booking.getShowTime()) ||
                    oldBooking.getNumberOfTickets() != booking.getNumberOfTickets()) {

                // Get the movie to calculate the new total price
                Optional<Movie> movie = movieService.getMovieById(booking.getMovieId());
                if (movie.isPresent()) {
                    booking.setTotalPrice(movie.get().getTicketPrice() * booking.getNumberOfTickets());
                }
